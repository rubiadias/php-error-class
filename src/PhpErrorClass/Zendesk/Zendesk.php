<?php
namespace PhpErrorClass\Zendesk;
/**
...
class Zendesk
{
    ...
    public function __construct($apiKey, $user, $subDomain, $suffix = '.json', $test = false)
    {
        self::$api_key = $apiKey;
        self::$user    = $user;
        self::$base    = 'https://' . $subDomain . '.zendesk.com/api/v2';
        self::$suffix  = $suffix;
        if ($test === true && !self::$test())
        {
    ...
    {
        if (substr_count($url, self::$suffix) == 0)
        {
            $url .= '.json';
        }
        ...
        curl_setopt($ch, CURLOPT_URL, self::$base.$url);
        curl_setopt($ch, CURLOPT_USERPWD, self::$user."/token:".self::$api_key);
        switch($action){
            case "POST":
